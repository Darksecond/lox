...
fn parse_infix<'a, It>(it: &mut Peekable<It>, left: Expr) -> Result<Expr, String>
    where It: Iterator<Item=&'a Token> {
        match peek(it)? {
            ...
            &Token::Or |
            &Token::And => parse_logical(it, left),
...
mod tests {
    ...
    #[test]
    fn test_unary() {
        assert_eq!(parse_str("-nil"), Ok(
            Expr::Unary(UnaryOperator::Minus, Box::new(Expr::Nil))
        ));
        assert_eq!(parse_str("!nil"), Ok(
            Expr::Unary(UnaryOperator::Bang, Box::new(Expr::Nil))
        ));
        assert_eq!(parse_str("!!nil"), Ok(
            Expr::Unary(UnaryOperator::Bang, Box::new(
                Expr::Unary(UnaryOperator::Bang, Box::new(Expr::Nil))
            ))   
        ));
        assert_eq!(parse_str("!-nil"), Ok(
        ...
        assert_eq!(parse_str("-!nil"), Ok(
            Expr::Unary(UnaryOperator::Minus, Box::new(
                Expr::Unary(UnaryOperator::Bang, Box::new(Expr::Nil))
            ))   
        ));
    }
    ...
    #[test]
    fn test_binary() {
        assert_eq!(parse_str("1!=2"), Ok(make::simple_binary(BinaryOperator::BangEqual)));
        assert_eq!(parse_str("1==2"), Ok(make::simple_binary(BinaryOperator::EqualEqual)));
        assert_eq!(parse_str("1>2"),  Ok(make::simple_binary(BinaryOperator::Greater)));
        assert_eq!(parse_str("1>=2"), Ok(make::simple_binary(BinaryOperator::GreaterEqual)));
        assert_eq!(parse_str("1<2"),  Ok(make::simple_binary(BinaryOperator::Less)));
        assert_eq!(parse_str("1<=2"), Ok(make::simple_binary(BinaryOperator::LessEqual)));
        assert_eq!(parse_str("1+2"),  Ok(make::simple_binary(BinaryOperator::Plus)));
        assert_eq!(parse_str("1-2"),  Ok(make::simple_binary(BinaryOperator::Minus)));
        assert_eq!(parse_str("1*2"),  Ok(make::simple_binary(BinaryOperator::Star)));
        assert_eq!(parse_str("1/2"),  Ok(make::simple_binary(BinaryOperator::Slash)));
    }

    #[test]
    fn test_binary_precedence() {
        use self::make::*;
        assert_eq!(parse_str("1*2+3*4"), Ok(
            binary(
        ...
        assert_eq!(parse_str("-1*-2"), Ok(
            binary(minus_nr(1.), BinaryOperator::Star, minus_nr(2.))
        ));
    }
    ...
    fn test_errors() {
        ...
        assert_eq!(parse_str("1+2 3"),  Ok(make::simple_binary(BinaryOperator::Plus)));
    ...
    fn test_grouping() {
        ...
        assert_eq!(parse_str("(1+2)*(1+2)"),  Ok(
            binary(
                Expr::Grouping(Box::new(
                    simple_binary(BinaryOperator::Plus)
                )),
                BinaryOperator::Star,
                Expr::Grouping(Box::new(
                    simple_binary(BinaryOperator::Plus)
        ...
        assert_eq!(parse_str("(1"), Err("No more tokens".into()));
        assert_eq!(parse_str("(1}"), Err("Expected RightParen got RightBrace".into()));
    }

    #[test]
    fn test_logical() {
        assert_eq!(parse_str("true or false"), Ok(
            Expr::Logical(
        ...
        assert_eq!(parse_str("true and false"), Ok(
            Expr::Logical(
                Box::new(Expr::Boolean(true)),
                LogicalOperator::And,
                Box::new(Expr::Boolean(false)),
    ...
    fn test_logical_precedence() {
        assert_eq!(parse_str("1 and 2 or 3 and 4"), Ok(
            Expr::Logical(
                Box::new(
                    Expr::Logical(
    ...
    #[test]
    fn test_assignment() {
        assert_eq!(parse_str("a=3"), Ok(
            Expr::Assign("a".into(), Box::new(Expr::Number(3.)))
        ));
        assert_eq!(parse_str("a=b=3"), Ok(
            Expr::Assign(
                "a".into(), 
                Box::new(
                    Expr::Assign(
        ...
        assert_eq!(parse_str("a=1+2"), Ok(
            Expr::Assign(
                "a".into(), 
                Box::new(
                    make::simple_binary(BinaryOperator::Plus)
                )
            )
        ));
    }
    ...
    fn test_call() {
        ...
        assert_eq!(parse_str("a(3)+a(3)"), Ok(
            Expr::Binary(
                Box::new(Expr::Call(
                    Box::new(Expr::Variable("a".into())),
                    vec![
        ...
        assert_eq!(parse_str("a(3,)"), Err("unexpected token: RightParen".into()));
    }

    #[test]
    fn test_get() {
        assert_eq!(parse_str("a.b"), Ok(
            Expr::Get(
                Box::new(Expr::Variable("a".into())),
                "b".into(),
            )
        ...
        assert_eq!(parse_str("a.b(3).c"), Ok(
            Expr::Get(
                Box::new(Expr::Call(
                    Box::new(Expr::Get(
                        Box::new(Expr::Variable("a".into())), 
